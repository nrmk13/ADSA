...
import { COLOCATED_GUIDE, defaultGuideDirs, workspaceDirs } from "./config.mjs";
...
export function scan(root, config, repoRoot = root) {
    ...
    const guideDirs = resolveGuideDirs(root, config, repoRoot);
...
function guideSlug(file) {
    return kebab(basename(file).replace(/\.mdx?$/, "").replace(/\.(spec|docs?|guide)$/i, ""));
}
