...
.badge.strong i{background:#166534}.badge.fair i{background:#15803d}.badge.weak i{background:#a16207}.badge.poor i{background:#b91c1c}
...
<main>
  ...
  <section class="panel" id="fix">
    ...
    <p class="after"><s>then</s><span>Run <code>adsa audit</code> again. The distance between the two runs is the deliverable.</span></p>
  </section>
  ...
  <section class="grid g-2 stretch" id="keep">
    <div class="panel">
      ...
      <div class="badge ${band}" role="img" aria-label="Badge preview: agent-ready ${scored.total} of ${scored.max}"><b>agent-ready</b><i>${scored.total}/${scored.max}</i></div>
      <p class="badge-note">how it renders in the README</p>
      ${snippet(`![Agent-ready](https://img.shields.io/badge/agent--ready-${scored.total}%2F${scored.max}-${badgeColor(band)})`)}
    </div>
    <div class="panel">
...
function todoRows(items) {
    ...
    return items
        .map((item) => {
            const command = `adsa fix ${item.fix}`;
...
const badgeColor = (band) => (band === "strong" ? "166534" : band === "fair" ? "15803d" : band === "weak" ? "a16207" : "b91c1c");
...
export function markdown(facts, scored) {
    ...
    for (const i of items) {
        lines.push(`- [ ] **${i.label}** — ${i.dimension} ${i.from} → ${i.to}, ${KIND_WORD[i.kind]}. \`adsa fix ${i.fix}\``);
    }
