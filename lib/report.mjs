...
.brand{font-family:var(--mono);font-size:11px;letter-spacing:.14em;text-transform:uppercase;color:var(--ink-3)}
...
<aside class="rail">
  <div class="brand">Agent readiness</div>
  <div class="who">
    <b>${esc(facts.name)}</b>
...
<main>
  ...
  <div class="grid g4">
    ${stat(scoreClass(band), `${scored.total}/${scored.max}`, "Agent readiness", delta === null ? `rubric ${RUBRIC.version}` : `${delta >= 0 ? "+" : ""}${delta} since the last run`)}
    ...
    ${stat(facts.a11y.ratio >= 0.9 ? "good" : facts.a11y.ratio >= 0.3 ? "warn" : "bad", String(facts.a11y.withSection), "Guides with keyboard docs", `${pct(facts.a11y.ratio)} of ${facts.a11y.total} guides`)}
  </div>
